...
class CategoryWiseVideoActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        categoryWiseVideo(sortby, categoryID)


    }

    private fun categoryWiseVideo(sort_by: String, categoryID: String) {
        if (page == 1)
            showProgress()
        val retrofitHelper = RetrofitHelper(activity)
        var call: Call<ResponseBody> =
            retrofitHelper.api().categoryWiseVideo(
                sort_by, categoryID, page
            )
                        ...
                        override fun onLoadMore() {
                            if (page < totalPage) {
                                ...
                                Handler(Looper.getMainLooper()).postDelayed({
                                    ...
                                    page += 1
                                    categoryWiseVideo(sort_by, categoryID)
